...
int manhattan_dist(const Point3i& p1, const Point3i& p2) {
    auto[x,y,z] = p2 - p1;
...
int main() {
    ...
    int largest_manhattan_dist = 0;
    
    for (int i = 0; i < readings.size(); ++i) {
        for (int j = i+1; j < readings.size(); ++j) {
            largest_manhattan_dist = max(largest_manhattan_dist, manhattan_dist(readings[i].origin, readings[j].origin));
        }
    }
    
    cout << largest_manhattan_dist << endl;
}
