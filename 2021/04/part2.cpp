...
int main() { 
  ...
  for (int num : numbers) {
      vector<Board> boards_after;
      boards_after.reserve(boards.size());
      for (Board& board : boards) {
          if (!mark_and_check(board, num)) {
              boards_after.push_back(board);
          }
      }
      if (boards_after.size() == 0) {
          int ans = sum_unmarked(boards.back())*num;
          cout << ans << endl;
          return 0;
      }
      boards = move(boards_after);
  }
