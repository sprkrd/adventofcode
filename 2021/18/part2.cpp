...
int main() {
    ...
    while (getline(cin, line)) {
        numbers.push_back(parse_snail_number(line));
    }
    
    SnailNumber* op1 = nullptr;
    SnailNumber* op2 = nullptr;
    int largest_mag = 0;
    SnailNumber* largest_sum = nullptr;
    
    for (auto num1 : numbers) {
        for (auto num2 : numbers) {
            if (num1 != num2) {
                auto sum = num1->sum(num2);
                int mag = sum->get_magnitude();
                if (mag > largest_mag) {
                    delete largest_sum;
                    op1 = num1;
                    op2 = num2;
