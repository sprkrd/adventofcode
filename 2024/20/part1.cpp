...
int main()
{
    constexpr size_t saveAtLeast = 100;
    ...
    for (size_t i = 0; i < pathToEnd.size(); ++i)
    {
        ...
        {
            if (manhattanDistance(pathToEnd[i], pathToEnd[j]) == 2)
            {
                ++ans;
                // int timeCheat = i + (pathToEnd.size()-j-1) + 2;
                // int savedTime = (pathToEnd.size()-1) - timeCheat;
                // cheats[savedTime]++;
            }
        }
    }
