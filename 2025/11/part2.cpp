...
size_t CountPaths(const Graph& graph, const vector<string>& sorted, const string& u, const string& v, const string& w = "")
{
    unordered_map<string, size_t> countRegister{{u, 1}};
    ...
    return countRegister[v];
}

int main()
{
    Graph input = Input();
    auto sorted = TopoSort(input, "svr");
    size_t ans =
        CountPaths(input, sorted, "svr", "dac", "fft")*
        CountPaths(input, sorted, "dac", "fft")*
        CountPaths(input, sorted, "fft", "out", "dac");
    ans +=
        CountPaths(input, sorted, "svr", "fft", "dac")*
        CountPaths(input, sorted, "fft", "dac")*
        CountPaths(input, sorted, "dac", "out", "fft");
    cout << ans << endl;
}
