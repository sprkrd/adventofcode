...
Graph support_graph(const vector<Brick>& bricks) {
    ...
    for (int i = 0; i < (int)bricks.size(); ++i) {
        for (auto&&[x,y,z] : bricks[i].all_cubes()) {
            ...
            if (h==z-1 && support!=-1 && support!=i && !contains(graph[support],i)) {
                graph[support].push_back(i);
            }
...
vector<int> compute_in_degree(const Graph& graph) {
...
int calculate_sum_falling_bricks(const Graph& graph, vector<int> in_degree, int seed) {
    int ans = 0;
    queue<int> open;
    open.push(seed);
    while (!open.empty()) {
...
int calculate_sum_falling_bricks(const Graph& graph) {
    auto in_degree = compute_in_degree(graph);
    int ans = 0;
    for (int i = 0; i < (int)graph.size(); ++i) {
        ans += calculate_sum_falling_bricks(graph, in_degree, i);
    }
    return ans;
}
...
int main() {
    ...
    cout << calculate_sum_falling_bricks(graph) << endl;
}
