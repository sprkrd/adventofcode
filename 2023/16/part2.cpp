...
Map energize(const Map& input, const BeamState& init) {
    unordered_set<BeamState, BeamStateHasher> closed;
    vector<BeamState> stk{init};
    while (!stk.empty()) {
        auto state = stk.back();
        stk.pop_back();
...
int main() {
    ...
    int max_energized = 0;
    for (int x = 0; x < map.ncols; ++x) {
        max_energized = max(max_energized, energize(map, {{x,0},Direction::south}).count('#'));
        max_energized = max(max_energized, energize(map, {{x,map.nrows-1},Direction::north}).count('#'));
    }
    for (int y = 0; y < map.nrows; ++y) {
        max_energized = max(max_energized, energize(map, {{0,y},Direction::east}).count('#'));
        max_energized = max(max_energized, energize(map, {{map.ncols-1,y},Direction::west}).count('#'));
    }
