...
int main() {
    ...
    //for (auto action : actions) {
        //int reward = state.step(blueprints[0], action);
        //acc_reward += reward;
        //std::cout << state << std::endl;
        //std::cout << "last action scored " << reward << " geodes" << std::endl;
    //}

    //std::cout << acc_reward << std::endl;
    
    //State state(24);
    //Optimize optimize(blueprints[1]);
    //std::cout << state << std::endl;
    //auto[action,value] = optimize(state);
    //while (action != done) {
        //state.step(blueprints[1], action);
        //std::cout << state << std::endl;
        //std::tie(action,std::ignore) = optimize(state);
    //}
    //state.step(blueprints[1], done);
    //std::cout << state << std::endl;
    ...
    for (const Blueprint& blueprint : blueprints) {
        ...
        int quality = geodes*blueprint.index;
        sum_quality += quality;
    }

    std::cout << sum_quality << std::endl;
}
