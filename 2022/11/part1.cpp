...
struct Expression {
    ...
    integer eval(integer old_value) {
        integer lhs_eval = lhs == k_old? old_value : lhs;
        integer rhs_eval = rhs == k_old? old_value : rhs;
        switch (operation) {
            case add: return lhs_eval + rhs_eval;
            case mul: return lhs_eval * rhs_eval;
...
struct System {
    ...
    void step(Monkey& monkey) {
        int need_to_inspect = monkey.items.size();
        
        for (int i = 0; i < need_to_inspect; ++i) {
            integer item_score = monkey.items.front();
            item_score = monkey.operation.eval(item_score);
            item_score /= 3;
    ...
    int monkey_business() const {
        vector<int> inspections(monkeys.size());
