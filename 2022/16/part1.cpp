...
struct Optimize {
    ...
    Optimize(const Graph& graph) :
        graph(graph), cache(1000, Optimize::hash_args) {}

    int operator()(int minutes_remaining, const string& valve_id) {
        return (*this)(minutes_remaining, graph.at(valve_id).id, 0);
    }
