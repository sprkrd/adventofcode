...
pair<int,State> bfs(Environment& env, State start, Point2i goal) {
    env.set_state(start);
...
int main() {
    ...
    env.init(cin);
    //cout << env << endl;
    auto start_position = env.char_at;
    auto[a_to_b_1, state_1] = bfs(env, env.get_state(), env.goal);
    auto[b_to_a_2, state_2] = bfs(env, state_1, start_position);
    auto[a_to_b_3, ignored] = bfs(env, state_2, env.goal);
    int ans = a_to_b_1 + b_to_a_2 + a_to_b_3;
    cout << ans << endl;
}
