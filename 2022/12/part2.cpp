...
int bfs(const Map& map) {
    ...
    while (!open.empty() && distance[get<0>(end)][get<1>(end)] == inf) {
        ...
        open.pop();
        int tentative_dist = distance[get<0>(current)][get<1>(current)] + 1;
        for (Point2i neigh : map.get_neighbors(current)) {
            if (distance[get<0>(neigh)][get<1>(neigh)] <= tentative_dist)
                continue;
            distance[get<0>(neigh)][get<1>(neigh)] = tentative_dist;
            open.emplace(neigh);
        }
    }
